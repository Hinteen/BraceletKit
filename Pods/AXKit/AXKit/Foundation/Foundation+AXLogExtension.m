...
static NSString *getLogPath(){
    ...
    logPath = [[@"com.xaoxuu.AXKit" stringByAppendingPathComponent:logFileDir] stringByAppendingPathComponent:fileName].cachePath;
...
+ (NSString *)getFunc:(const char *)func{
    return [NSString stringWithFormat:@"##### [%@] func:%s", [getDateFormatter() stringFromDate:[NSDate date]], func];
}
...
+ (void)writeLogWithFunc:(NSString *)func input:(NSObject *)input{
    ...
    if (!queue_log) {
        queue_log = dispatch_get_global_queue(0, 0);
    }
