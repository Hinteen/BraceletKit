...
- (instancetype)init{
    if (self = [super init]) {
        ...
        _model = [AppInfoModel mj_objectWithKeyValues:@"appinfo".extension(@"json").mainBundlePath.readJson()];
...
- (NSString *)randomTips{
    if (!footerArray) {
        NSMutableArray *arrM = [NSMutableArray arrayWithArray:@"footer_rhesis".extension(@"json").mainBundlePath.readJson()];
        if (arrM) {
